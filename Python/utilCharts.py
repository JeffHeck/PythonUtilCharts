...
# Get df of fitted curve data using weights
def getDfFittedCurve(aDataDF, aX, aY, aW, aOrder, aGeneratedPointCount):
    #print('getDfFittedCurve')
    # Filter out NAs
    aDataDF  = aDataDF.loc[aDataDF[aX].notnull()]
    aDataDF  = aDataDF.loc[aDataDF[aY].notnull()]
    # Generate fitted curve equation using weights
    if (aW == None) | (aW == ''):
        z = np.polyfit(x=aDataDF[aX], 
                       y=aDataDF[aY], deg=aOrder)
    else:
        # First filter out NANs. Then use the weights.
        aDataDF  = aDataDF.loc[aDataDF[aW].notnull()]
        z = np.polyfit(x=aDataDF[aX], 
                       y=aDataDF[aY], deg=aOrder,
                       w=aDataDF[aW])
    p = np.poly1d(z)
    # Create fitted curve data for the number of requested points
    fittedCurveDF = pd.DataFrame({'x': list(range(0,aGeneratedPointCount))})
    minX = min(aDataDF[aX])
    maxX = max(aDataDF[aX])
    diffX = maxX - minX
    delta = diffX / aGeneratedPointCount
    fittedCurveDF['x'] = minX + fittedCurveDF['x'] * delta
    fittedCurveDF['y'] = p(fittedCurveDF['x'])
    return fittedCurveDF
...
def plotBarStacked(aXValues, aYValues, aDF, aSave=False, 
    ...
    plt.gcf().subplots_adjust(bottom=aBottomSpacing)
    barType = 'bar'
    if aVerticalBars == False:
        barType = 'barh'
    if aUseColors:
        myplot=aDF.plot(kind=barType, stacked=True, x=aXValues, width=aBarWidth,
                        figsize=(aXFigSize, aYFigSize), rot=aXLabelRotation, fontsize=aLabelFontSize, color=aColors)
    else:
        myplot=aDF.plot(kind=barType, stacked=True, x=aXValues, width=aBarWidth,
                        figsize=(aXFigSize, aYFigSize), rot=aXLabelRotation, fontsize=aLabelFontSize)
    myplot.yaxis.grid(aYGrid)
    myplot.xaxis.grid(aXGrid)
    ...
    if (aFormatYlabel):
        myplot.get_yaxis().set_major_formatter(plt.FuncFormatter(lambda x, loc: aYLabelFormat.format(int(x))))
    if (aFormatXlabel):
        myplot.get_xaxis().set_major_formatter(plt.FuncFormatter(lambda x, loc: aXLabelFormat.format(int(x))))
...
# General func to plot scatter and optionally include a fitted curve w weights
# Params:
#   aW = Column with weights
#   aCurveFitPointCount = Number of points to generate for the fitted curve
def plotScatter(aPlotDF, aX, aY, aHighlightList, aHighlightCol, aW=None,
                aTitle='', 
                aXTitle='', aYTitle='',
                aSaveFilename='ReportsAutoGenerated/Scatter.png', 
                aLegendLocation=2, aCurveFitOrder=2,
                aCurveFitPointCount=100,
    ...
    if aUseCircleSize:
     ...
     # Plot data with no fitted curve first
    sns.regplot(y=aY,
                x=aX,
                data=aPlotDF,
                scatter_kws={'s': circleSize, 'alpha':0.2}, fit_reg=False,
                order=aCurveFitOrder, ci=None, truncate=True)
     # Over lay fitted curve data
    if aIncludeFittedCurve:
        fittedCurveDf = getDfFittedCurve(aDataDF=aPlotDF, aX=aX, aY=aY, 
                                         aW=aW, aOrder=aCurveFitOrder, 
                                         aGeneratedPointCount=aCurveFitPointCount)
        sns.lineplot(y='y',
                     x='x',
                     data=fittedCurveDf,
                     color='k')
    # Add the highlighted entries and legend
    ...
    fig.set_size_inches(aXFigSize, aYFigSize)
    ax.set_xlabel(aXTitle, fontsize=16)
    ax.set_ylabel(aYTitle, fontsize=16)
    if aFormatXLabel:
        ax.get_xaxis().set_major_formatter(plt.FuncFormatter(lambda x, loc: aXLabelFormat.format(int(x))))
...
def plotScatterMulti(aPlotDF0, aX0, aY0, aHighlightList0, aHighlightCol0,
                     ...
                     aW0=None, aW1=None, aCurveFitPointCount=100,
    ...
    sns.regplot(y=aY0,
                ...
                color=aColor0,
                label=aLegendTitle0,
                scatter_kws={'s': circleSize0, 'alpha':0.2}, fit_reg=False,
                order=aCurveFitOrder, ci=None, truncate=True)
     # Over lay fitted curve data
    if aIncludeFittedCurve:
        fittedCurveDf = getDfFittedCurve(aDataDF=aPlotDF0, aX=aX0, aY=aY0, 
                                         aW=aW0, aOrder=aCurveFitOrder, 
                                         aGeneratedPointCount=aCurveFitPointCount)
        sns.lineplot(y='y',
                     x='x',
                     data=fittedCurveDf,
    ...
    sns.regplot(y=aY1,
                ...
                color=aColor1,
                label=aLegendTitle1,
                scatter_kws={'s': circleSize1, 'alpha':0.2}, fit_reg=False,
                order=aCurveFitOrder, ci=None, truncate=True)
    if aIncludeFittedCurve:
        fittedCurveDf = getDfFittedCurve(aDataDF=aPlotDF1, aX=aX1, aY=aY1, 
                                         aW=aW1, aOrder=aCurveFitOrder, 
                                         aGeneratedPointCount=aCurveFitPointCount)
        sns.lineplot(y='y',
                     x='x',
                     data=fittedCurveDf,
